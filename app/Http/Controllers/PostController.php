...
class PostController extends Controller
{
    ...
    public function backend_remove_posting(Request $request)
    {
        ...
        unlink(substr($request->thumbnail,23));
    ...
    {
        if ($request->ajax()) {
            $data = Posting::orderBy('id','desc')->with('jenisposting','kategoriposting','penulisposting','sumberposting');
    ...
    public function backend_kategori(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function($data){
                    $actionBtn = ' <button data-id="'.$data->id.'" data-name="'.$data->kategori_name.'" data-toggle="modal" data-target="#modaledit" class="delete btn btn-info btn-sm"><i class="text-white fa fa-pencil"></i></button>';
